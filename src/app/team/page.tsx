...
function TeamMemberCard({ member, index }: { member: TeamMember; index: number }) {
  const gradientColor = roleColors[member.role] || "from-purple-400 to-pink-400";
    ...
    >
      <div className="relative h-full bg-gradient-to-br from-purple-900/30 via-black/40 to-purple-900/20 backdrop-blur-sm rounded-xl border border-purple-500/20 p-4 sm:p-6 hover:border-purple-400/40 transition-all duration-300">
        ...
        <div className="relative mx-auto mb-3 sm:mb-4">
          <div className={`w-20 h-20 sm:w-24 sm:h-24 mx-auto rounded-full bg-gradient-to-br ${gradientColor} p-0.5`}>
            <div className="w-full h-full rounded-full bg-[#0E1A2B] flex items-center justify-center">
              <span className={`font-heading text-2xl sm:text-3xl font-bold bg-gradient-to-br ${gradientColor} bg-clip-text text-transparent`}>
                {member.name.charAt(0)}
              </span>
            </div>
          </div>
          {/* Glow behind avatar */}
          <div className={`absolute inset-0 w-20 h-20 sm:w-24 sm:h-24 mx-auto rounded-full bg-gradient-to-br ${gradientColor} opacity-20 blur-xl -z-10`} />
        </div>
        ...
        <p className={`font-accent text-[10px] sm:text-xs text-center bg-gradient-to-r ${gradientColor} bg-clip-text text-transparent font-medium mb-3 sm:mb-4`}>
          {member.role}
        </p>
        ...
        {/* Bottom Accent */}
        <div className={`mt-4 sm:mt-5 h-0.5 bg-gradient-to-r ${gradientColor} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left rounded-full`} />
      </div>
    </motion.div>
  );
}
